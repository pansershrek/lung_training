...
class CustomModelCheckpoint(Callback):
    ...
    def check_monitor_top_k(self, current):
        ...
        if abs((current - self.best_k_models[self.kth_best_model_path]) ) < 1e-6:
            return False
        return monitor_op(current-(1e-3), self.best_k_models[self.kth_best_model_path])
    ...
    def on_validation_end(self, trainer, pl_module):
        ...
        if self.save_top_k != -1:
            ...
            if current is None:
                ...
                log.info(f'\nEpoch {epoch:05d}: {self.monitor} {current.item():.4f} was not in top {self.save_top_k}')
    ...
    def _do_check_save(self, filepath, current, epoch, trainer, pl_module):
        ...
        if self.verbose > 0:
            ...
            log.info(
                ...
                f' {current.item():.5f} (best {self.best_model_score.item():.5f}), saving model to'
