...
class YoloV4Loss(nn.Module):
    def __init__(self, config=None, iou_threshold_loss=0.02):
        super(YoloV4Loss, self).__init__()
        self.__iou_threshold_loss = iou_threshold_loss
        self.config = None if config==None else GetPBB(config) # seems no use at first glance

    def forward(self, predict, target):
        self.get_predict_d(predict)
